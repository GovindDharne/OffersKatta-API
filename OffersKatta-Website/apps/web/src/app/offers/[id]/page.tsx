...
import { Bell, BellOff, ChevronLeft, ChevronRight, Heart, MapPin, Navigation, QrCode, Share2, Tag } from 'lucide-react';
        ...
        <div className="space-y-2">
          ...
          {o.branch?.brand ? (
            <div className="flex flex-wrap items-center gap-3 text-muted-foreground">
              <p>by <span className="font-medium text-foreground">{o.branch.brand.name}</span></p>
              <FollowBrandButton brandId={o.branch.brand.id} brandName={o.branch.brand.name} />
            </div>
          ) : null}
...
/// Customer-only Follow toggle next to the brand name. Logged-out users see
/// a "Log in to follow" link; sellers/admins see nothing (it's a customer
/// signal, not relevant to them).
function FollowBrandButton({ brandId, brandName }: { brandId: string; brandName: string }) {
  const { user, isAuthenticated } = useAuth();
  const qc = useQueryClient();
  const isCustomer = isAuthenticated && user?.role === 'CUSTOMER';

  const statusQ = useQuery({
    queryKey: ['brand-follow-status', brandId],
    queryFn: () => apiGet<{ following: boolean; followerCount: number }>(`/brands/${brandId}/follow/status`),
    enabled: isCustomer,
    staleTime: 30_000,
  });
  ...
  const toggle = useMutation({
    mutationFn: async () => {
      if (statusQ.data?.following) {
        await apiDelete(`/brands/${brandId}/follow`);
        return { following: false };
      } else {
        await apiPost(`/brands/${brandId}/follow`, {});
        return { following: true };
      }
    },
    onSuccess: (next) => {
      qc.invalidateQueries({ queryKey: ['brand-follow-status', brandId] });
      toast.success(next.following
        ? `Following ${brandName} — you'll get a ping on new offers.`
        : `Unfollowed ${brandName}.`);
    },
    onError: (e) => toast.error((e as Error).message ?? 'Could not update follow.'),
  });

  if (!isAuthenticated) {
    return (
      <a href={`/login?next=/offers`} className="text-xs text-primary hover:underline">
        Log in to follow
      </a>
    );
  }
  ...
  const count = statusQ.data?.followerCount ?? 0;
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        {following ? <BellOff className="mr-1 h-3 w-3" /> : <Bell className="mr-1 h-3 w-3" />}
        {following ? 'Following' : 'Follow'}
      </Button>
      {count > 0 ? (
        <span className="text-xs text-muted-foreground">{count} follower{count === 1 ? '' : 's'}</span>
      ) : null}
    </div>
  );
}
